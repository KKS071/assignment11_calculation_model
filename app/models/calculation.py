...
class AbstractCalculation:
    """Mixin with shared columns and logic for all calculation types."""
    ...
    @declared_attr
    def user_id(cls):
        # FK to users table; cascade delete removes calculations when user is deleted
        return Column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'),
                      nullable=False, index=True)

    @declared_attr
    def type(cls):
        # Discriminator column used by SQLAlchemy for polymorphic identity
        return Column(String(50), nullable=False, index=True)

    @declared_attr
    def inputs(cls):
        # Stores variable-length list of numbers as JSON
    ...
    def create(cls, calculation_type: str, user_id: uuid.UUID,
        ...
        """Factory method: returns the correct Calculation subclass based on type."""
        calculation_classes = {
            'addition': Addition,
    ...
    def get_result(self) -> float:
        """Override in subclasses to compute the calculation result."""
...
class Calculation(Base, AbstractCalculation):
    """Base SQLAlchemy model with polymorphic inheritance configured."""
    __mapper_args__ = {
        "polymorphic_on": "type",
...
class Division(Calculation):
    """Divides the first number by each subsequent number sequentially."""
